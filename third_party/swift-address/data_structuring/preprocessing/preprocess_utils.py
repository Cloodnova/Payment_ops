...
def filter_and_save_alternate_names(
        ...
        f.write(
            zlib.compress(
                orjson.dumps(
                    {
                        town: aliases[0]
                        for town, aliases in pl.scan_csv(
                        ...
                        .with_columns(
                            ...
                            pl.col("isColloquial").replace_strict(
                                old=["0", "1"], new=[False, True], default=False
                            ),
                            pl.col("isHistoric").replace_strict(
                                old=["0", "1"], new=[False, True], default=False
                            ),
                        )
                        .filter(
                        ...
                        .with_columns(
                            ...
                            pl.col("cc_list").replace_strict(
                                old=languages, default=default_languages
                            )
                        )
                        .filter(
                            ...
                            (
                                ...
                                & (pl.col("isolanguage").is_in(pl.col("cc_list")))
                            )
                            | (pl.col("alternate name").is_null())
                        )
                        .with_columns(
                            # Cleanup the remaining alternate names
                            alternate_name=decode_and_clean_expr(pl.col("alternate name"))
                            .str.strip_chars()
                            .str.to_uppercase()
                            .fill_null("")
                        )
                        .with_columns(
                            ...
                            alternate_names=(
                                pl.concat_list(
                                    ...
                                    .implode()
                                    .over(partition_by=partitioning_col)
                                    .list.unique(),
                                    pl.col(name_column),
                                )
                                .list.unique()
...
def get_country_data(country_info_path: Path, additional_languages: list[str]) -> pl.LazyFrame:
    ...
    with open(country_info_path, "r") as f:
        ...
        return (
            ...
            .with_columns(pl.col("geonameid").str.to_integer(), pl.col("Languages").str.split(","))
            .explode("Languages")
            ...
            .agg(
                pl.concat_list(pl.col("Languages").implode(), additional_languages)
                .list.unique()
                .alias("Languages"),
                pl.all().exclude("Languages"),
            )
...
def get_country_languages(df_country_data: pl.LazyFrame) -> dict[str, list[str]]:
    return {
        iso: languages[0]
        for iso, languages in df_country_data.select("ISO", "Languages")
        .collect(engine="streaming")
