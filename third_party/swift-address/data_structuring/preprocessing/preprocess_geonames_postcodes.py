...
CHAR_TO_REGEX = (
    {char: "[A-Z]" for char in string.ascii_uppercase}  # if any character except numbers or space
...
def filter_and_clean_dataframe(input_df: pl.LazyFrame) -> pl.LazyFrame:
    return (
        input_df.with_columns(
            # Cleanup post codes
            postal_code_filtered=pl.col("postal code")
            .str.to_uppercase()
        ...
        .with_columns(
            # Cleanup town names
            place_name_filtered=(
                decode_and_clean_expr(pl.col("place name").str.to_uppercase()).str.replace_all(
                    r"[^A-Z0-9]", " "
                )
            )
        )
        ...
        .with_columns(
            ...
            country_code_list=pl.col("country code")
            .unique()
            .implode()
            .over(partition_by="postal_code_filtered"),
            # Obtain list of place names per filtered post code
            place_name_list=pl.col("place name")
            .unique()
            .implode()
            .over(partition_by="postal_code_filtered"),
            # Obtain list of decoded place names per filtered post code
            place_name_filtered_list=(
                pl.col("place_name_filtered")
                .unique()
                .implode()
                .over(partition_by="postal_code_filtered")
            ),
        ...
        .with_columns(
            ...
            post_code_mapping=pl.col("country_code_list")
            .list.unique()
            .list.concat(["place_name_filtered_list"])
        )
        .group_by(
        ...
        f.write(
            zlib.compress(
                orjson.dumps(
                    {
                        k: v[0]
                        for k, v in df.select("postal_code_filtered", "post_code_mapping")
                        .rows_by_key(key=["postal_code_filtered"], unique=True)
                        .items()
                    }
                ),
                level=-1,
...
def preprocess_ireland(input_df: pl.LazyFrame) -> pl.LazyFrame:
    return pl.concat(
        [
            input_df,
            input_df.filter(pl.col("place name").str.contains(r"(?i)Dublin")).with_columns(
                # Dublin uses multiple post codes, so these need to be duplicated to
                # represent the different ways of writing them
                ("D" + pl.col("postal code").str.slice(1).str.strip_chars_start("0")).alias(
                    "postal code"
                )
            ),
        ]
...
if __name__ == "__main__":
    ...
    combined_df = pl.concat(
        [
            pl.scan_csv(
                cli_args.input_geonames_postcodes_all_countries_path,
        ...
        + [
            ...
            for file_path in [
                cli_args.input_geonames_postcodes_ca_full_path,
    ...
    preprocess_and_save(
        input_df=all_countries_df.filter(
            ~pl.col("country code").is_in(COUNTRIES_WITH_SPECIAL_POST_CODES)
        ),
