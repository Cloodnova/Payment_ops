...
def split_every_35th_char(string):
    ...
    for word in re.split(r"(\W)", string):
        ...
        if word not in SEPARATOR_CHARS:
            if len(split_words) == 0 or any(x in split_words[-1] for x in SEPARATOR_CHARS):
    ...
    while i < len(split_words):
        if split_words[i] == "\n":
            ...
            if len(split_words[i]) <= MOVE_WORD_NEXT_LINE_LENGTH and (
                35 < len(current_line + split_words[i]) <= 35 + MOVE_WORD_NEXT_LINE_LENGTH
            ):
                res = res + current_line + "\n"
                current_line = ""
                continue
