...
class BaseSettingsISO(BaseSettings, ABC):
    ...
    model_config = SettingsConfigDict(
        env_prefix="ds_", cli_parse_args=True, cli_ignore_unknown_args=True
    )
...
class DatabaseConfig(BaseSettingsISO):
    ...
    postcodes_almost_all_countries_regex_list: Path = (
        "post_codes/almost_all_countries_regex_list.json"
    )
...
class CRFConfig(BaseSettingsISO):
    ...
    model_weights_path: Path = (
        resources.files(data_structuring.__name__)
        / ".."
        / "resources"
        / "models"
        / "CRF_with_MLP_EPOCH_1.safetensors"
    )
    ...
    vocabulary: list[str] = (
        list(string.ascii_uppercase) + list(string.digits) + list(r"/-?:().,'+ {}") + ["\r", "\n"]
    )
...
class RunCLIConfig(BaseSettingsISO):
    ...
    output_data_path: Path = Field(
        Path("data_structuring_output.csv"),
    ...
    logging_config: Path | None = Field(
        default=None,
...
class PreprocessPostcodesCLIConfig(BaseSettingsISO):
    ...
    input_geonames_postcodes_all_countries_path: Path = Field(
        ...
        alias=AliasChoices("i_ac", "input_geonames_postcodes_all_countries_path"),
    )
