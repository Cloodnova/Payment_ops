...
def _cli():
    ...
    if cli_args.input_data_path.suffix == ".txt":
        ...
        logger.error(
            "Unsupported file format %s. Please provide a .txt, .csv or .tsv",
            cli_args.input_data_path,
        )
        sys.exit(1)
    ...
    if cli_args.output_data_path.suffix in (".csv", ".tsv"):
        _, saved_path = ResultPostProcessing.save_list_as_human_readable_csv(
            results,
        ...
        exclude = (
            {"__all__": {"crf_result": {"emissions_per_tag", "log_probas_per_tag"}}}
            if not cli_args.verbose
            else None
        )
        saved_path = ResultPostProcessing.save_list_as_json(
            results, file_name=cli_args.output_data_path, indent=2, exclude=exclude
        )
    else:
