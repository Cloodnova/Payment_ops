...
from data_structuring.components.models.encoder_transformer import (
    EncoderTransformer as BackboneTransformer,
)
...
class ResultCRF(BaseModel):
    grouped_tags: Details = Field(
        description="An annotated `Details` object that "
        "contains detailed information about the prediction"
    )
    emissions: TorchTensor = Field(
        description="Emissions produced by the Transformer of shape (SEQUENCE_LENGTH, NUM_TAGS)"
    )
    log_likelihood: TorchTensor = Field(
        description="Log-likelihood produced by the CRF of shape (SEQUENCE_LENGTH, NUM_TAGS)"
    )
...
class TransformerCRF(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = torch.nn.Parameter(
            pos_embed_1d(
                d_model, (max_seq_len + 1) if self._has_country_classifier else max_seq_len
            ).unsqueeze(0),
            requires_grad=False,
        )

        # Transformer encoder layers: Add context to the embeddings through self-attention
        self.transformer_encoder = BackboneTransformer(
            d_model=d_model,
            nhead=nhead,
            mlp_ratio=mlp_ratio,
            dropout=dropout,
        ...
        if self._has_country_classifier:
            self.country_predictor = CountryHead(
                embedding_dim=d_model, num_countries=len(mapping_id_to_country)
            )
    ...
    def _produce_embeddings(self, sentences, mask):
        ...
        if self._has_country_classifier:
            ...
            true_mask = torch.ones(
                embeddings.shape[0], 1, dtype=torch.bool, device=embeddings.device
            )
        ...
        embeddings = self.transformer_encoder(src=embeddings, src_key_padding_mask=~mask)
    ...
    def loss(self, sentences, gt_country_ids, tags, mask):
        ...
        with suppress(RuntimeError):
            reg += self.regularisation_transitions_order_2 * torch.sum(
                self.crf.transitions_order_2**2
            )
    ...
    def forward(self, sentences, gt_country_ids=None, tags=None, mask=None):
        ...
        if tags is None:  # Inference (no ground truth tags are given)
            ...
            if self._has_country_classifier:
                country_preds_indices, country_preds_confidences = self.country_predictor.predict(
                    cls_token
                )
    ...
    def predict_tags(
        ...
        with torch.inference_mode():
            ...
            # Padding
            all_padded_words_ids = [
                (word_ids + (self._max_seq_len - len(word_ids)) * [self._padding_idx])
            ...
            # Produce tags
            tags_estimated, emissions, cls_token, _ = self.decode(
                padded_words_ids, mask, emissions_forced=emissions_forced
            )
            ...
            for i, _ in enumerate(all_tags):
                if len(all_tags[i]):
                    grouped_tags = create_details_from_biotags(
                        ...
                        country=self._mapping_id_to_country[country_preds_indices[i]]
                        if country_preds_indices is not None
                        else None,
                        country_confidence=country_preds_confidences[i]
                        if country_preds_confidences
                        else None,
                        tags=all_tags[i],
