...
class CRF(BaseCRF):
    ...
    def forward(
        self, emissions: torch.Tensor, tags: torch.LongTensor, mask: torch.ByteTensor | None = None
    ) -> torch.Tensor:
        if mask is None:
    ...
    def _denominator_score(self, emissions: torch.Tensor, mask: torch.ByteTensor) -> torch.Tensor:
        ...
        for i in range(1, sequence_length):
            ...
            # Transition scores
            transition_scores = self.get_transitions().view(
                1, num_tags, num_tags
            )  # (1, num_tags, num_tags)
            # Broadcast alpha
...
class CRFSecondOrder(CRF):
    ...
    def __init__(
        self, num_tags: int, padding_idx: int | None = None, batch_first: bool = True
    ) -> None:
...
if __name__ == "__main__":
    ...
    model2.transitions_order_2 = torch.nn.Parameter(torch.ones_like(model2.transitions_order_2))
