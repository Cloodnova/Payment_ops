...
def create_details_from_biotags(
    raw_content: str,
    country: str,
    ...
    grouped_spans = (
        _get_strict_span(grouped_bio_tags, pad_with_other)
    ...
    if pad_with_other:
        ...
        if previous_end != len(raw_content):
            grouped_spans.append(
                TaggedSpan(start=previous_end, end=len(raw_content), tag=Tag.OTHER)
            )
...
def _get_strict_span(grouped_bio_tags: list, pad_with_other: bool) -> list[TaggedSpan]:
    ...
    for ((start, _), bio_tag_1), ((_, end), bio_tag_2) in zip(
        grouped_bio_tags, grouped_bio_tags[1:]
    ):
        if (
            bio_tag_1.tag == bio_tag_2.tag
            and bio_tag_1.bio == BIO.BEFORE
            and bio_tag_2.bio == BIO.INSIDE
        ):
...
def _get_span(grouped_bio_tags: list) -> list[TaggedSpan]:
    ...
    for (start, end), bio_tag in grouped_bio_tags:
        if bio_tag.tag == current_bio_tag.tag:
            ...
            grouped_spans.append(
                TaggedSpan(start=current_start, end=current_end, tag=current_bio_tag.tag)
            )
    ...
    grouped_spans.append(
        TaggedSpan(start=current_start, end=grouped_bio_tags[-1][0][1], tag=current_bio_tag.tag)
    )
    return grouped_spans
