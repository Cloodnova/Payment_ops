...
def load_country_dict(
    config: DatabaseConfig, country_provinces: dict
        ...
        with open(config.country_groupings) as f:
            ...
            overrides = list(
                set(
                    [
                        country
                        for grouping in config.force_country_groupings
                        if grouping in country_groupings
                        for country in country_groupings[grouping]
                    ]
                    + config.force_countries
                )
            )
        logger.info("Loaded %i countries from %i groups", len(overrides), len(country_groupings))
    else:
