...
SAINT_VARIATIONS_REGEX = re.compile(
    r"\bSAINT([^\s\w]"
    r"|\s)|\bST\.([^\s\w]|\s)"
    r"|\bST([^\s\w]|\s)"
)

SAINT_VARIATIONS = ["SAINT-", "ST. ", "ST-"]
...
def decode_and_clean_expr(expr: pl.Expr) -> pl.Expr:
    ...
    """
    return expr.map_elements(DECODE_ALIAS, return_dtype=pl.String()).str.replace_many(
        CHAR_REPLACEMENT
    )
...
def generate_duplicate_aliases(name):
    ...
    return [name] + [
        name_alias
        for variation in duplicate_if_saint_in_name(name)
