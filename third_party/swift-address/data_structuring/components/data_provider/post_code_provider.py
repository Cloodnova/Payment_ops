...
def load_postcode_data(config: DatabaseConfig):
    ...
    logger.info(
        "Loaded %i postcodes and %i regex for IRELAND", len(ireland_dict), len(ireland_regex_list)
    )

    malta_dict = load_file(config.postcodes_maltese_dict)
    malta_regex_list = load_file(config.postcodes_maltese_regex_list)
    logger.info(
        "Loaded %i postcodes and %i regex for MALTA", len(malta_dict), len(malta_regex_list)
    )

    chile_dict = load_file(config.postcodes_chilean_dict)
    chile_regex_list = load_file(config.postcodes_chilean_regex_list)
    logger.info(
        "Loaded %i postcodes and %i regex for CHILE", len(chile_dict), len(chile_regex_list)
    )
    ...
    logger.info(
        "Loaded %i postcodes and %i regex for ARGENTINA",
        len(argentina_dict),
    ...
    logger.info(
        "Loaded %i postcodes and %i regex for BRAZIL", len(brazil_dict), len(brazil_regex_list)
    )

    china_dict = load_file(config.postcodes_chinese_dict)
    china_regex_list = load_file(config.postcodes_chinese_regex_list)
    logger.info(
        "Loaded %i postcodes and %i regex for CHINA", len(china_dict), len(china_regex_list)
    )
