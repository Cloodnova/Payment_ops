...
def get_extended_towns(
    config: DatabaseConfig, towns_to_remove: dict, country_override: list[str] | None = None
...
def _load_from_osm(
    ...
    extended_all_possibilities_town: dict[str, dict[str, set[str]]] = defaultdict(
    ...
    df_osm = (
        ...
        .filter(
            ...
            (pl.col("population") >= config.town_minimal_population)
            | (pl.col("iso").is_in(country_override)),
        )
        .with_columns(
        ...
        .select(
            ...
            pl.all()
            .get(0)
            .over(partition_by="decoded_city_name", order_by="population", descending=True)
        )
        .group_by(
        ...
        .with_columns(
            ...
            decoded_original_city_name=decode_and_clean_expr(
                pl.col("original_city_name")
            ).map_elements(lambda x: x.casefold(), return_dtype=pl.String())
        )
        .with_columns(
            # Calculate damerau levenshtein distance between both town names
            decoded_city_name_distance=pl.struct(
                "decoded_original_city_name", "decoded_city_name"
            ).map_elements(
                lambda x: damerau_levenshtein_distance(
                    x["decoded_original_city_name"], x["decoded_city_name"]
                ),
    ...
    return (
        extended_all_possibilities_town,
        extended_towns_and_populations,
        extended_largest_country_code_for_town,
    )
