...
def towns_from_geonames(config: DatabaseConfig) -> tuple[dict, dict, dict]:
    ...
    df_geonames = (
        ...
        .with_columns(
            ...
            .unique()
            .implode()
            .over(partition_by="decoded_name")
        )
        .select(
            # Keep only the highest population count for each town
            pl.all()
            .get(0)
            .over(partition_by="decoded_name", order_by="population", descending=True)
        )
        ...
        for dupe_name in decoded_name_duplicates:
            ...
            if alias_names:  # Skip empty lookups
                for name in alias_names:
                    ...
                    all_possible_aliases.update(
                        decode_and_clean_str(alias).casefold() for alias in name_aliases
                    )
