...
class CharacterTokenizer:
    def __init__(
        ...
        assert all(len(char) == 1 for char in vocabulary), "All characters must be of length 1"
    ...
    def encode(self, string: str):
        return [self._char_to_idx.get(char, self._char_to_idx[self._unk_token]) for char in string]
