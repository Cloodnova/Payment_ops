...
class FuzzyMatch(BaseModel):
    ...
    start: NonNegativeInt = Field(
        description="The absolute index of the first letter of the match within the message"
    )
    end: NonNegativeInt = Field(
        description="The absolute index of the last letter of the match within the message"
    )
    ...
    flags: list[BaseFlag] = Field(
        default_factory=list,
        description="The list of all the flags that were raised for this match",
    )
    origin: str | None = Field(
        default=None, description="The country from which `possibility` originates."
    )
    # CRF-related attributes
    crf_score: FiniteFloat | None = Field(default=None, description="The CRF score of this match")
    transformer_score: FiniteFloat | None = Field(
        default=None, description="The transformer score of this match"
    )
    # Final score attribute
    final_score: FiniteFloat | None = Field(
        default=None,
...
def fuzzyscan_all_batched(
    ...
    matrix_results = cdist(
        queries, match_against, scorer=partial_ratio, score_cutoff=score_cutoff, workers=n_workers
    )
        ...
        for candidate in candidates:
            ...
            for match in all_matches:
                ...
                try:
                    ...
                    query_match = (
                        match.matched[:-1] if match.matched.endswith(".") else match.matched
                    )

                    match_is_standalone = bool(re.search(rf"\b{query_match}\b", input_text))
                except Exception:
