...
class Details(BaseModel, frozen=True):
    ...
    country_code_confidence: float | None = Field(
        default=None,
        description="The confidence of the country code, if it represents a prediction",
    )
    ...
    def color_print(self, add_legend: bool = True):
        ...
        for tagged_span in self.spans:
            print(
                f"{colored.back(tag_to_color[tagged_span.tag])}{self.content[tagged_span.start : tagged_span.end]}",
                end="",
            )
            color_used_str.add(
                f"{colored.back(tag_to_color[tagged_span.tag])}{tagged_span.tag.value}"
            )
