    ...
    def read(self) -> Generator[AddressSample, None, None] | None:
        ...
        try:
            for chunk in pl.scan_csv(
                self.file_path, separator=self.sep, encoding=self.encoding, infer_schema=False
            ).collect_batches(chunk_size=10000, maintain_order=True, lazy=True, engine="streaming"):
            ...
            raise ValueError(
                f"Column '{self.data_column_name}' not found in CSV file: {self.file_path}"
            ) from e
