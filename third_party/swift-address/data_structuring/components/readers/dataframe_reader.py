...
from data_structuring.components.readers.base_reader import (
    BaseReader,
    AddressSample,
...
class DataFrameReader(BaseReader):
    def __init__(
        ...
        self.suggested_country_column = (
            suggested_country_column if suggested_country_column in columns_in_df else None
        )
        self.force_suggested_country_column = (
            force_suggested_country_column
    ...
    def read(self) -> Generator[AddressSample, None, None]:
        ...
        if self.suggested_country_column:
            filtered = filtered.with_columns(
                (
                    pl.col(self.suggested_country_column)
                    .cast(pl.String)
                    .str.strip_chars()
                    .str.to_uppercase()
                )
            ...
            filtered = filtered.with_columns(pl.lit(None).alias(self.suggested_country_column))

        if self.force_suggested_country_column:
            filtered = filtered.with_columns(
                (
                    pl.col(self.force_suggested_country_column)
                    .cast(pl.String)
                    .str.to_lowercase()
                    .str.strip_chars()
                    .is_in(POSSIBLE_FORCED_FLAG_VALUES)
                    .fill_null(False)
                )
            )
        else:
