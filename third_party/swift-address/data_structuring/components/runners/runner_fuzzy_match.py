...
class RunnerFuzzyMatch(BaseRunner):
    ...
    def match(
        self, data: list[str], suggested_countries: list[str | None] | None = None
        ...
        all_fs_countries = fuzzy_scan.fuzzyscan_all_batched(
            data,
        ...
        all_fs_country_codes = fuzzy_scan.fuzzyscan_all_batched(
            data,
        ...
        all_fs_towns = fuzzy_scan.fuzzyscan_all_batched(
            data,
        ...
        all_word_mapping_towns_extended = [
            {
                ...
                for iso in set(
                    [
                        match.origin
                        for match in fuzzy_scan.FuzzyMatchResult.merge(
                            fs_countries, fs_country_code
                        )
        ...
        all_extended_fs_towns = [
            fuzzy_scan.fuzzyscan_all_batched(
                [sample],
        ...
        for country_res, country_code_res, town_res, extended_town_res in zip(
            all_fs_countries, all_fs_country_codes, all_fs_towns, all_extended_fs_towns
        ):
