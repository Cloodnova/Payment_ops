...
from data_structuring.components.runners.post_processing.combination_generator import (
...
class RunnerPostProcessing(BaseRunner):
    ...
    def __init__(
        ...
        self.combination_generator = CombinationGenerator(
            database, config, town_weights, country_weights
        )

    def filter_fuzzy_results(
        self, fuzzymatch_result: FuzzyMatchResult, original_text_length: int
    ) -> None:
        """
        ...
        self.inclusion_flagger.flag_matches_included_in_another(
            fuzzymatch_result, fuzzymatch_result
        )
    ...
    def split_country_list_in_code_and_not_code(
        self, country_list: FuzzyMatchResult
    ...
    def run(
        ...
        for crf_result, fuzzy_match_result, postcode_match_result, address_sample in zip(
            ...
            country_marginal_emission_by_token = (
                crf_result.emissions_per_tag[Tag.COUNTRY].cpu().numpy().astype(np.float64)
            )
            country_marginal_logprobability_by_token = (
                crf_result.log_probas_per_tag[Tag.COUNTRY].cpu().numpy().astype(np.float64)
            )
            town_marginal_emission_by_token = (
                crf_result.emissions_per_tag[Tag.TOWN].cpu().numpy().astype(np.float64)
            )
            town_marginal_logprobability_by_token = (
                crf_result.log_probas_per_tag[Tag.TOWN].cpu().numpy().astype(np.float64)
            )
            ...
            if suggested_country:
                for match in fuzzy_match_result.country_code_matches:
                    if suggested_country == match.possibility and suggested_country == match.origin:
                        ...
                        match.crf_score = max(
                            match.crf_score, self.config.base_score_suggested_country
                        )

                if suggested_country != "NO COUNTRY":
                    suggested_country_match = FuzzyMatch(
                        start=len(address_sample.text) + 2,
                        end=len(address_sample.text) + 2,
                        matched="",
                        dist=0,
                        origin=suggested_country,
                    ...
                    fuzzy_match_result.country_code_matches = FuzzyMatchResult.merge(
                        fuzzy_match_result.country_code_matches,
                        FuzzyMatchResult([suggested_country_match]),
                    )
            ...
            fuzzy_match_result.country_matches = FuzzyMatchResult.merge(
                fuzzy_match_result.country_matches, fuzzy_match_result.country_code_matches
            )
            ...
            fuzzy_match_result.town_matches = FuzzyMatchResult.merge(
                fuzzy_match_result.town_matches, fuzzy_match_result.extended_town_matches
            )
            self.match_scorer.score_matches_with_emissions(
            ...
            self.country_flag_manager.add_all_flags(
                fuzzy_match_result, crf_result, sample, sample_casefold, ibans
            )
            ...
            self.inclusion_flagger.flag_matches_included_in_another(
                queries=country_codes_result_list, larger_matches=fuzzy_match_result.country_matches
            )
            ...
            country_head = (
                crf_result.details.country_code
                if (
                    crf_result.details.country_code
            ...
            self.relationship_manager.add_relationship_flags(
                fuzzy_match_result, sample, country_head
            )
            ...
            for postal_result in postcode_match_result.postcode_matches:
                for town_match in fuzzy_match_result.town_matches:
                    for postal_result_town_alias in generate_duplicate_aliases(
                        postal_result.possibility
                    ):
                        if (
                            postal_result_town_alias == town_match.possibility
            ...
            for country_result in fuzzy_match_result.country_matches:
                ...
                country_result.final_score = self.score_computer.compute_country_score(
                    country_result.crf_score, country_result.dist, country_result.flags
                )
            for town_result in fuzzy_match_result.town_matches:
                ...
                town_result.final_score = self.score_computer.compute_town_score(
                    town_result.crf_score, town_result.dist, town_result.flags
                )
            ...
            countries_above_threshold = [
                match
                for match in fuzzy_match_result.country_matches
                if match.final_score >= self.config.minimal_final_score_country
            ]

            towns_above_threshold = [
                match
                for match in fuzzy_match_result.town_matches
            ...
            country_town_combinations = self.combination_generator.generate_combinations(
                countries_above_threshold, towns_above_threshold, no_country, no_town
            )
