...
class RunnerCRF(BaseRunner):
    ...
    def __init__(self, config: CRFConfig, database: Database):
        ...
        use_country_predictor = any(
            "country_predictor" in weight_name for weight_name in model_weights
        )
        # Load model configuration from JSON file
        with open(self.config.model_config_path, "r") as fp:
            self.model_config = orjson.loads(fp.read())
        # Convert tags to correct type
        self.model_config["mapping_id_to_country"] = {
            int(key): iso for key, iso in self.model_config["mapping_id_to_country"].items()
        }
        self.model_config["tags_to_keep"] = [Tag(tag) for tag in self.model_config["tags_to_keep"]]
        self.model_config["bio_tags_to_keep"] = [
            BIOTag(**tag) for tag in self.model_config["bio_tags_to_keep"]
        ]
        ...
        self.model = TransformerCRF(
            ...
            regularisation_transitions_order_2=self.model_config[
                "regularisation_transitions_order_2"
            ],
    ...
    def _get_log_probas_and_emissions_B_I(self, tag, emissions, marginal_probas):
        ...
        if tag == Tag.OTHER:
            ...
            saved_marginal_probas = (
                marginal_probas[:, row_numbers[0]] + marginal_probas[:, row_numbers[1]]
            )
    ...
    def tag(self, data: list[str]) -> Iterator[ResultRunnerCRF]:
        ...
        predictions = self.model.predict_tags(
            data, tokenizer=self.tokenizer, strict_before_inside=False
        )
        ...
        for result, sample in zip(predictions, data):
            ...
            for span in result.grouped_tags.spans:
                logprobs_per_tag[span.tag], emissions_per_tag[span.tag] = (
                    self._get_log_probas_and_emissions_B_I(
                        span.tag, result.emissions, result.log_likelihood
                    )
                )
            ...
            for tag in self.model_config["tags_to_keep"]:
                if tag not in predictions_per_tag:
                    ...
                    logprobs_per_tag[tag] = torch.zeros(
                        self.model_config["max_sequence_length"], device=result.emissions.device
                    )
                    emissions_per_tag[tag] = torch.zeros(
                        self.model_config["max_sequence_length"], device=result.emissions.device
                    )
