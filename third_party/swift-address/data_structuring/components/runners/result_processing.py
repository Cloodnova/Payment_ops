...
class ResultRunnerCRF(BaseModel):
    details: Details = Field(
        description="An annotated `Details` object that contains detailed"
...
class ResultRunnerFuzzyMatch(BaseModel):
    country_matches: fuzzy_scan.FuzzyMatchResult = Field(
        description="A list of all `FuzzyMatch` detected for the countries"
    )
    country_code_matches: fuzzy_scan.FuzzyMatchResult = Field(
        description="A list of all `FuzzyMatch` detected for the country codes"
    )
    town_matches: fuzzy_scan.FuzzyMatchResult = Field(
        description="A list of all `FuzzyMatch` detected for the towns"
    )
    extended_town_matches: fuzzy_scan.FuzzyMatchResult = Field(
        description="A list of all `FuzzyMatch` detected for the extended towns"
    )
    ...
    match obj:
        ...
        case dict():
            return (
                "["
                + ", ".join(
                    f"{_to_str(key, depth + 1)}={_to_str(value, depth + 1)}"
...
class ResultPostProcessing(BaseModel):
    ...
    fuzzy_match_result: ResultRunnerFuzzyMatch = Field(
        description="The result of the fuzzymatch runner on the message"
    )
    ibans: list[str] = Field(
        description="A list of the IBANs that have been identified in the message"
    )
    suggested_country: str | None = Field(
        default=None, description="The suggested country code from the input"
    )
    force_suggested_country: bool = Field(
        default=False,
        description="Whether the suggested country was forced as the only possibility",
    )

    def _i_th_best_match(
        self,
        i: int,
        ...
        if town_or_country == Tag.COUNTRY:
            ...
            raise ValueError(
                f"`town_or_country` should be {Tag.COUNTRY} or {Tag.TOWN}, got {town_or_country}."
            )

        if len(matches) > i:
            i_th_match = matches[i]
            return (
                getattr(i_th_match, resolved_attr),
                i_th_match.final_score,
    ...
    @staticmethod
    def save_list_as_json(
        list_results: list[ResultPostProcessing],
        ...
        **kwargs_model_dump_json,
        ...
        json_str = _root_list_of_result_post_processing(list_results).model_dump_json(
            **kwargs_model_dump_json
        )
    ...
    @staticmethod
    def save_list_as_human_readable_csv(
        list_results: list["ResultPostProcessing"],
        ...
        for result in list_results:
            ...
            for i in range(n_best_matches):
                (best_match_country, best_confidence_country, best_match_raw_country) = (
                    result.i_th_best_match_country(i=i)
                )
                (best_match_town, best_confidence_town, best_match_raw_town) = (
                ...
                if isinstance(best_confidence_country, float):
                    current_row[f"{i + 1}th_best_country_confidence"] = (
                        f"{round(best_confidence_country * 100, confidence_precision)}%"
                    )
                    current_row[f"{i + 1}th_best_country_resolved_code"] = best_match_country
                ...
                if isinstance(best_confidence_town, float):
                    current_row[f"{i + 1}th_best_town_confidence"] = (
                        f"{round(best_confidence_town * 100, confidence_precision)}%"
                    )
                    current_row[f"{i + 1}th_best_town_resolved"] = best_match_town
                else:  # No confidence (i.e, no detection)
            ...
            if verbose:
                ...
                for tag in verbose_crf_tags:
                    current_row[f"crf_prediction_{tag.value.lower()}"] = _to_str(
                        sorted(
                            list(result.crf_result.predictions_per_tag[tag]), key=lambda x: x.start
                        )
                    )
