...
class BaseFlagManager:
    ...
    def _add_separator_typo_flag(match: FuzzyMatch) -> None:
        ...
        if match.dist > 0:
            if BaseFlagManager._normalize_string(
                match.matched
            ) == BaseFlagManager._normalize_string(match.possibility):
...
class MatchInclusionFlagger:
    ...
    def flag_matches_included_in_another(
        queries: list[FuzzyMatch], larger_matches: FuzzyMatchResult
    ) -> None:
        """
...
class TownFlagManager(BaseFlagManager):
    ...
    def add_all_flags(
        self, fuzzy_match_result: ResultRunnerFuzzyMatch, crf_result: ResultRunnerCRF
    ) -> None:
    ...
    def check_alone_on_line(
        self, fuzzy_match_result: ResultRunnerFuzzyMatch, original_address: str
    ) -> None:
...
class CountryFlagManager(BaseFlagManager):
    ...
    def _add_feature_flags(
        self, match: FuzzyMatch, crf_result: ResultRunnerCRF, sample: str, sample_casefold: str
    ) -> None:
        ...
        if domain_extensions and any(
            domain_ext in sample_casefold for domain_ext in domain_extensions
        ):
...
class RelationshipFlagManager:
    ...
    def _check_pair_validity(
        self, town_match: FuzzyMatch, country_match: FuzzyMatch, original_address: str
    ) -> None:
        """Check if country-town pair is valid and add appropriate flags (optimized)."""
        # Skip if not reliable matches
        if (
            country_match.dist > 0
            or town_match.dist > 0
        ...
        if (
            CommonFlag.IS_SHORT in country_match.flags
        ...
        if (
            CountryFlag.IS_SUGGESTED_COUNTRY in country_match.flags
        ...
        if (
            CommonFlag.IS_COMMON_STATE_PROVINCE_ALIAS in country_match.flags
        ...
        if (
            character_distance <= 15
            and CountryFlag.GENERATED_BY_SUGGESTED_COUNTRY not in country_match.flags
        ):
    ...
    def check_reasonable_mistakes(
        self, fuzzy_match_result: ResultRunnerFuzzyMatch, crf_result: ResultRunnerCRF
    ) -> None:
        """Flag matches that could have been confused by CRF."""
        for town_match in fuzzy_match_result.town_matches:
            if any(
                town_match.matched in pred.prediction
        ...
        for country_match in fuzzy_match_result.country_matches:
            if any(
                country_match.matched in pred.prediction
