...
class CombinationGenerator:
    ...
    def generate_combinations(
        ...
        combinations.extend(
            self._generate_matched_pairs(countries_above_threshold, towns_above_threshold)
        )

        # Countries without matching towns
        combinations.extend(self._generate_solo_countries(countries_above_threshold, no_town))

        # Towns without matching countries
        combinations.extend(self._generate_solo_towns(towns_above_threshold, no_country))
        ...
        if not combinations:
            combinations.append(
                (
                    no_country,
                    no_town,
                    (self.config.minimal_final_score_country + self.config.minimal_final_score_town)
                    / 2,
                )
            )
    ...
    def _generate_matched_pairs(
        self, countries: list[FuzzyMatch], towns: list[FuzzyMatch]
    ) -> list[tuple[FuzzyMatch, FuzzyMatch, float]]:
    ...
    def _should_skip_pair(self, town_match: FuzzyMatch, country_match: FuzzyMatch) -> bool:
        ...
        if (
            town_match.start == country_match.start
            and town_match.end == country_match.end
            and self.database.country_town_same_name.get(town_match.possibility, "")
            != country_match.origin
        ):
            return True

        # One is subset of the other
        if (
            (town_match.start > country_match.start and town_match.end <= country_match.end)
    ...
    def _generate_solo_countries(
        self, countries: list[FuzzyMatch], no_town: FuzzyMatch
    ) -> list[tuple[FuzzyMatch, FuzzyMatch, float]]:
        ...
        for country_match in countries:
            ...
            cumulative_malus = self.config.no_town_found_mul * sum(
                [
                    self.country_weights.town_is_present
                    if (CountryFlag.TOWN_IS_PRESENT in country_match.flags)
                    else 0,
                    self.country_weights.is_very_close_to_town
                    if (CountryFlag.IS_VERY_CLOSE_TO_TOWN in country_match.flags)
                    else 0,
                    self.country_weights.is_on_same_line_as_town
                    if (CountryFlag.IS_ON_SAME_LINE_AS_TOWN in country_match.flags)
                    else 0,
                ]
            )

            score = (
                country_match.final_score + self.config.minimal_final_score_town - cumulative_malus
            ) / 2
    ...
    def _generate_solo_towns(
        self, towns: list[FuzzyMatch], no_country: FuzzyMatch
    ) -> list[tuple[FuzzyMatch, FuzzyMatch, float]]:
        """Generate combinations for towns without matching countries."""
        combinations = []

        if (
            self.suggested_country
            and self.force_suggested_country
            and self.suggested_country != "NO COUNTRY"
        ):
            return combinations
        ...
        for town_match in towns:
            cumulative_malus = self.config.no_country_found_mul * sum(
                [
                    ...
                    if (TownFlag.COUNTRY_IS_PRESENT in town_match.flags)
                    else 0,
                    self.town_weights.suggested_country_is_present_bonus
                    if (TownFlag.SUGGESTED_COUNTRY_IS_PRESENT in town_match.flags)
                    else 0,
                    self.town_weights.is_very_close_to_country
                    if (TownFlag.IS_VERY_CLOSE_TO_COUNTRY in town_match.flags)
                    else 0,
                    self.town_weights.is_on_same_line_as_country
                    if (TownFlag.IS_ON_SAME_LINE_AS_COUNTRY in town_match.flags)
                    else 0,
                ]
            )

            score = (
                self.config.minimal_final_score_country + town_match.final_score - cumulative_malus
            ) / 2
    ...
    def _deduplicate_combinations(
        self, combinations: list[tuple[FuzzyMatch, FuzzyMatch, float]]
    ) -> list[tuple[FuzzyMatch, FuzzyMatch, float]]:
