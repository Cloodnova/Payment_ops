...
from data_structuring.components.data_provider import (
    load_country_dict,
    towns_from_geonames,
    load_postcode_data,
    get_extended_towns,
...
class Database:
    ...
    def _load_all_data(self):
        ...
        (self.alpha2_to_alpha2, self.all_possibilities_country) = load_country_dict(
            config=self.config, country_provinces=self.provinces
        )
        ...
        (
            ...
            self.largest_country_code_for_town,
        ) = towns_from_geonames(config=self.config)
        ...
        self.full_all_possibilities_town.update(
            {
                town: iso_list
                for iso, towns in self.extended_all_possibilities_town.items()
        ...
        (
            self.full_dict,
            self.full_regex_list,
            self.ireland_dict,
            self.ireland_regex_list,
            self.malta_dict,
            self.malta_regex_list,
            self.chile_dict,
            self.chile_regex_list,
            self.argentina_dict,
            self.argentina_regex_list,
            self.brazil_dict,
            self.brazil_regex_list,
            self.china_dict,
            self.china_regex_list,
        ) = load_postcode_data(self.config)
