...
class AddressStructuringPipeline:
    ...
    def __init__(
        ...
        self._post_processing_town_weights_config = (
            post_processing_town_weights_config or PostProcessingTownWeightsConfig()
        )
        self._post_processing_country_weights_config = (
            post_processing_country_weights_config or PostProcessingCountryWeightsConfig()
        )
        ...
        self._fuzzy_runner = RunnerFuzzyMatch(
            config=self._fuzzy_match_config, database=self._database_controller
        )
        self._postcode_runner = RunnerPostcodeMatch(
            config=self._fuzzy_match_config, database=self._database_controller
        )
    ...
    def _clean_and_validate_sample(
        self, sample: AddressSample, raise_on_validation_error: bool = True
    ) -> AddressSample:
        ...
        try:
            ...
            if raise_on_validation_error:
                ...
                warnings.warn(
                    f"Unable to validate sample `{sample.text}`, this might lead to unexpected behaviors"
                )
    ...
    def run(self, reader: BaseReader, batch_size: int = 1024):
        ...
        for batch in _batched(samples, batch_size):
            ...
            all_fuzzy_match_results = self._fuzzy_runner.match(
                batch_texts, suggested_countries=[sample.suggested_country for sample in batch]
            )
